...
int pocitaniDelkySekvence(const char *src, int poziceSrc, int *delkaMezer, int *delkaNL, int *delkaSlova)
{
	switch (src[poziceSrc]) {
	case ' ':
		delkaMezer = strspn((src + poziceSrc), "\t ");/*pocita delku sekvence mezer a tabu*/
		break;
	case '\t':
		delkaMezer = strspn((src + poziceSrc), "\t ");/*pocita delku sekvence mezer a tabu*/
		break;
	case '\n':
		delkaNL = strspn((src + poziceSrc), "\n");/*pocita delku sekvence new line*/
	default:
                                                             ...
                                                             const char      * src )
{
	...
	while (src[poziceSrc] != 0) //cteni slov jednotlive
	{
		...
		delkaMezer = 0;
		delkaNL = 0;
		pocitaniDelkySekvence(*src, poziceSrc, &delkaMezer, &delkaNL, &delkaSlova);
